...
# 5. Kill any existing processes on ports 8000/8001, then start fresh
echo "[5/5] Starting uvicorn servers on ports 8000 and 8001..."
fuser -k 8000/tcp 2>/dev/null && echo "  Killed existing process on port 8000." || echo "  Port 8000 is free."
fuser -k 8001/tcp 2>/dev/null && echo "  Killed existing process on port 8001." || echo "  Port 8001 is free."
cd /app/backend
